...
int main(void)
{
  ...
  signed int out1;
  signed int out1_ref;
  mandel_data = fopen("mandel_test_data.txt", "r");
  while (!feof(mandel_data))
  {
    fscanf(mandel_data, "%08x ", &out1_ref);
    line_cnt++;
    mandel (&out1);
    if (out1 != out1_ref)
    {
  ...
  if (err_cnt == 0) {
    fprintf(stderr,"mandel passed all tests.\n");
  } else {
    fprintf(stderr,"mandel FAILED. Number of errors: %d\n", err_cnt);
  }
