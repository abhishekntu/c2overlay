...
architecture tb_arch of isqrt_tb is
  component isqrt
    port (
      clk : in std_logic;
      reset : in std_logic;
  ...
  file TestDataFile: text open read_mode is "isqrt_test_data.txt";
  file ResultsFile: text open write_mode is "isqrt_alg_test_results.txt";
begin
  uut : isqrt
    port map (
      clk => clk,
  ...
  ISQRT_BENCH: process
    ...
    variable outp_v, outp_ref : unsigned(31 downto 0);
    variable outp_v_vec, outp_ref_vec : std_logic_vector(31 downto 0);
  ...
  begin
    while not endfile(TestDataFile) loop
      ...
      -- Test ISQRT algorithm
      if (
          (outp_v /= outp_ref)
      ) then
        Passed := '0';
        write(Bufline, string'("ISQRT error: x="));
        ...
        writeline(ResultsFile, Bufline);
        ncycles_v := ncycles;
        write(Bufline, string'("ISQRT OK: Number of cycles="));
    ...
    if (Passed = '1') then
      write(Bufline, string'("ISQRT algorithm test has passed"));
      writeline(ResultsFile, Bufline);
      ...
      write(Bufline, string'("ISQRT algorithm test has not passed"));
      writeline(ResultsFile, Bufline);
      -- Automatic end of the current simulation.
      assert false
