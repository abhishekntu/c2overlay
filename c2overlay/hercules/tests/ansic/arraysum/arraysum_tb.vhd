...
entity arraysum_tb is
end arraysum_tb;

architecture tb_arch of arraysum_tb is
  component arraysum
    port (
      clk : in std_logic;
      reset : in std_logic;
  ...
  file TestDataFile: text open read_mode is "arraysum_test_data.txt";
  file ResultsFile: text open write_mode is "arraysum_alg_test_results.txt";
begin
  uut : arraysum
    port map (
      clk => clk,
  ...
  ARRAYSUM_BENCH: process
    ...
    variable ntests_max : integer := 10;
  ...
  begin
    while not endfile(TestDataFile) loop
      ...
      wait until done = '1';
      out1_v := signed(out1);
      -- Test ARRAYSUM algorithm
      if (
          (out1_v /= out1_ref)
      ) then
        Passed := '0';
        write(Bufline, string'("ARRAYSUM error: in1="));
        ...
        writeline(ResultsFile, Bufline);
        ncycles_v := ncycles;
        write(Bufline, string'("ARRAYSUM OK: Number of cycles="));
    ...
    if (Passed = '1') then
      write(Bufline, string'("ARRAYSUM algorithm test has passed"));
      writeline(ResultsFile, Bufline);
      ...
      write(Bufline, string'("ARRAYSUM algorithm test has not passed"));
      writeline(ResultsFile, Bufline);
      -- Automatic end of the current simulation.
      assert false
