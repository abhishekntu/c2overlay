...
-- Filename: fir_tb.vhd
-- Date: 12 October 2013 04:36:45 PM
...
architecture tb_arch of fir_tb is
  component fir
    port (
      clk : in std_logic;
      reset : in std_logic;
  ...
  file TestDataFile: text open read_mode is "fir_test_data.txt";
  file ResultsFile: text open write_mode is "fir_alg_test_results.txt";
begin
  uut : fir
    port map (
      clk => clk,
  ...
  begin
    while not endfile(TestDataFile) loop
      ...
      if (
        ...
        writeline(ResultsFile, Bufline);
        ncycles_v := ncycles;
        write(Bufline, string'("FIR OK: Number of cycles="));
    ...
    if (Passed = '1') then
      write(Bufline, string'("FIR algorithm test has passed"));
      writeline(ResultsFile, Bufline);
      ...
      write(Bufline, string'("FIR algorithm test has not passed"));
      writeline(ResultsFile, Bufline);
      -- Automatic end of the current simulation.
      assert false
