    ...
    MP_INT *b;
{
  ...
  mpz_init(&temp);
  mpz_set_ui(&temp,0);
  mpz_add_ui(&temp, a, 0UL);
  mpz_add_ui(a, b, 0UL);
  mpz_add_ui(b, &temp, 0UL);
  mpz_clear(&temp);
}
...
void mpz_printf(a, base)
    MP_INT *a;
    unsigned long base;
{
  printf("%s", mpz_get_str(NULL, base, a));
}


void mpz_fprintf(f, a, base)
    FILE *f;
    MP_INT *a;
    unsigned long base;
{
  fprintf(f, "%s", mpz_get_str(NULL, base, a));
}
